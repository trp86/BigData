...
def perform_dq_and_add_additional_columns(
    ...
    """Empty dataframe is created with schema present in input dataframe along with additional column rejectReason."""
    ...
    weather_data_column_details = list(
        map(
            lambda x: x.split(":"),
            config_dict["weather.metadata"]["columns"].split("|"),
        )
    )
    df_weather_with_T_values_replaced = transform.replace_T_with_negligible_values(
        df_weather_renamed, weather_data_column_details
    )

    # Typecast columns for weather data
    weather_data_typecasted = transform.typecastcolumns(
        df_weather_with_T_values_replaced, weather_data_column_details
    )

    # Data Quality check for weather data (Columns should not have negative value)
    weather_data_negative_check_columns = config_dict["weather.metadata"][
    ...
    (
        ...
        df=weather_data_typecasted,
        column_names=weather_data_negative_check_columns,
        error_empty_df=error_empty_df,
    )

    # Data Quality check for columns to be compared with certain value or any column in dataframe
    weather_data_columnsorvalue_check_columns = config_dict["weather.metadata"][
    ...
    (
        ...
        compare_expressions=weather_data_columnsorvalue_check_columns,
        error_empty_df=error_empty_df,
    )

    # Add additional columns
    success_df = add_rain_condition_column(
        add_snowdepth_condition_column(
            add_snowfall_condition_column(
                add_temperature_condition_column(success_df_columnsorvalue_check)
            )
        )
    )
...
def add_temperature_condition_column(df: DataFrame) -> DataFrame:
    """
    Column temperature_condition is added to the dataframe if averagetemperature column exists.
    ...
    return df.withColumn(
        ...
        when(col("averagetemperature") < 32, "verycold")
        .when(
            (col("averagetemperature") >= 32) & (col("averagetemperature") < 59), "cold"
        )
        .when(
            (col("averagetemperature") >= 59) & (col("averagetemperature") < 77),
            "normal",
        )
        .when(
            (col("averagetemperature") >= 77) & (col("averagetemperature") < 95), "hot"
        )
        .otherwise("veryhot"),
    )
...
def add_snowfall_condition_column(df: DataFrame) -> DataFrame:
    """
    Column snowfall_condition is added to the dataframe if snowfall column exists.
    ...
    return df.withColumn(
        "snowfall_condition",
        when(col("snowfall") <= 0.0001, "nosnow")
...
def add_snowdepth_condition_column(df: DataFrame) -> DataFrame:
    """
    Column snowdepth_condition is added to the dataframe if snowdepth column exists.
    ...
    return df.withColumn(
        "snowdepth_condition",
        when(col("snowdepth") <= 0.0001, "nosnow")
...
def add_rain_condition_column(df: DataFrame) -> DataFrame:
    """
    Column rain_condition is added to the dataframe if precipitation column exists.
    ...
    return df.withColumn(
        "rain_condition",
        when(col("precipitation") <= 0, "norain")
