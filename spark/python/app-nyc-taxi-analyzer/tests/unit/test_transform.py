...
def test_column_list_to_be_checked_return_none_if_column_exist(spark_session_test: SparkSession):
    # ASSEMBLE
    some_df = spark_session_test.createDataFrame([(1234,56.76), (2341,76.45)], schema='vendor_id string, total_amount string')
...
@pytest.mark.check_if_column_exists_in_df
def test_column_list_to_raise_error_if_column_not_exist(spark_session_test: SparkSession):
    # ASSEMBLE
    some_df = spark_session_test.createDataFrame([(1234,56.76), (2341,76.45)], schema='vendor_id string, total_amount string')
...
def test_rename_column_in_df_should_rename_column_if_column_exists(spark_session_test: SparkSession):
    # ASSEMBLE
    some_df = spark_session_test.createDataFrame([(1234,56.76), (2341,76.45)], schema='vendor_id string, total_amount string')
    expected_df = spark_session_test.createDataFrame([(1234,56.76), (2341,76.45)], schema='vendor_id string, net_amount string')
...
def test_rename_column_in_df_should_raise_error_if_column_not_exist(spark_session_test: SparkSession):
    # ASSEMBLE
    some_df = spark_session_test.createDataFrame([(1234,56.76), (2341,76.45)], schema='vendor_id string, total_amount string')
...
def test_typecastcolumns_should_typecast_dataframe_columns(spark_session_test: SparkSession):
    # ASSEMBLE
    ...
    input_df = spark_session_test.createDataFrame(test_spark_context.parallelize([
...
def test_typecastcolumns_should_throw_exception_if_unsupported_datatype_provided(spark_session_test: SparkSession):
    # ASSEMBLE
...
def test_filterrecordshavingnegativevalue_should_filter_out_records_having_negative_value(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
    ...
    error_empty_df = spark_session_test.createDataFrame(spark_session_test.sparkContext.emptyRDD(), input_df.schema).withColumn("rejectreason" , lit(""))
    ...
    # ASSERT
    expected_success_df = spark_session_test.createDataFrame(test_spark_context.parallelize([
       Row("CMT", 2, 2.5),
       Row("CMT", 0, 6.4), 
       Row("CMT", 1, 0.0)]), input_schema)

    expected_error_df = spark_session_test.createDataFrame(test_spark_context.parallelize([
...
def test_filterrecordshavingnegativevalue_should_throw_exception_if_column_is_not_present(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
...
def test_filter_records_having_improper_datetime_value_should_filterout_having_improper_datetime_value(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
...
@pytest.mark.filter_records_having_improper_datetime_value
# when filterrecordshavingnegativevalue function is invoked should throw exception if column is not present
def test_filter_records_having_improper_datetime_value_should_throw_exception_if_column_is_not_present(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
...
def test_df_columns_compare_should_filter_out_records_which_doesnot_match_compare_expression(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
...
def test_df_columns_compare_should_throw_exception_if_string_column_has_comparision_operator(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
...
def test_df_columns_compare_should_throw_exception_if_invalid_comparision_operator_is_used(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("vendor_id", StringType(), True),
...
def test_filter_records_having_improper_datetime_value_should_filterout_having_improper_datetime_value(spark_session_test: SparkSession):
    ...
    expected_df = spark_session_test.createDataFrame(test_spark_context.parallelize([
       Row("0.0001", "medium"), 
       Row("0.5000", "violent")]), input_schema) 
...
def test_replace_T_with_negligible_values_should_throw_exception_if_column_is_not_present(spark_session_test: SparkSession):
    # ASSEMBLE
    test_spark_context = spark_session_test.sparkContext
    input_schema = StructType([
        StructField("snowdepth", StringType(), True),
